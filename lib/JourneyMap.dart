  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: const Text(
            'My Journies', style: TextStyle(color: Colors.white),),
        ),
        body: _locations.isEmpty ? Center(
          child: Text(
            "No journals are here display", style: TextStyle(fontSize: 19),),
        )
            : FlutterMap(
          mapController: _mapController,
          options: MapOptions(initialCenter: _locations.isNotEmpty? _locations.first : LatLng(0, 0),
            initialZoom: 5.0,
          )
          // MapOptions(
